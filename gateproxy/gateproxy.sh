...
function gateproxy_setup(){
    ...
    crontab -l | { cat; echo "*/12 * * * * /etc/scr/bandata.sh"; } | crontab -
    ...
    crontab -l | { cat; echo "@daily sarg -l /var/log/squid/access.log -o /var/www/squid-reports &> /dev/null"; } | crontab -
    ...
    crontab -l | { cat; echo "#*/10 * * * * /etc/scr/banip.sh"; } | crontab -
...
${lang_22[${en}]} (y/n)" answer
    case $answer in
        [Yy]* )
            ...
            grep -qxF '*.none /var/log/samba/audit.log' /etc/rsyslog.conf || echo '*.none /var/log/samba/audit.log' | tee -a /etc/rsyslog.conf
            ...
            echo -e "local5.* /var/log/samba/audit.log\n& stop" | tee -a /etc/rsyslog.d/50-default.conf
...
crontab -l | { cat; echo "@reboot systemctl daemon-reload
...
@monthly journalctl --vacuum-size=500M && systemctl restart systemd-journald.service"; } | crontab -
echo OK
sleep 1
